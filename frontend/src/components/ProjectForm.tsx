  ...
  if (isViewMode) {
    return (
      <div className="space-y-6">
        {/* 2-Column Layout for Basic Info and Credentials */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-gray-700">Project Live URL</label>
              <div className="mt-1 flex items-center space-x-2">
                <div className="text-sm text-gray-900 flex-1">{formData.project_live_url || 'Not set'}</div>
                <button
                  type="button"
                  onClick={() => copyToClipboard(formData.project_live_url || 'Not set')}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700">Admin Login URL</label>
              <div className="mt-1 flex items-center space-x-2">
                <div className="text-sm text-gray-900 flex-1">{formData.admin_login_url || 'Not set'}</div>
                <button
                  type="button"
                  onClick={() => copyToClipboard(formData.admin_login_url || 'Not set')}
                  className="text-gray-500 hover:text-gray-700"
                  title="Copy to clipboard"
                >
                  <FaCopy />
            ...
            <div>
              ...
              <div className="mt-1 flex items-center space-x-2">
                <div className="text-sm text-gray-900 flex-1">{formData.username_email || 'Not set'}</div>
                <button
                  type="button"
                  onClick={() => copyToClipboard(formData.username_email || 'Not set')}
                  className="text-gray-500 hover:text-gray-700"
                  title="Copy to clipboard"
                >
                  <FaCopy />
        ...
        <div className="space-y-6">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700">Budget & Expenses</label>
            <div className="mt-1 flex justify-between items-start">
              <div className="text-sm text-gray-900">
                <div className="font-medium text-gray-700">Budget</div>
                <div className="mt-0.5">${formData.budget.toLocaleString()}</div>
              </div>
              {projectExpenses.length > 0 && (
                <div className="text-sm font-medium text-red-700 text-right">
                  <div className="font-medium">Total Expenses</div>
                  <div className="mt-0.5">${projectExpenses.reduce((total, expense) => total + Number(expense.amount), 0).toLocaleString()}</div>
                </div>
              )}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="mt-1">
              {existingFiles.length > 0 ? (
                ...
                <div className="text-sm text-gray-900">No files uploaded</div>
              )}
            </div>
          </div>
            ...
            <div className="mt-1 space-y-2">
              {loadingExpenses ? (
                ...
                projectExpenses.map((expense) => (
                  <div key={expense.id} className="flex items-center justify-between p-4 bg-white border rounded-lg shadow-sm hover:shadow-md transition-shadow">
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
